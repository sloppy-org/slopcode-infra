...
case "${MODEL_KEY}" in
  ...
  deepseek-v4-flash)
    ...
    export LLAMACPP_HOME="${LLAMACPP_HOME:-${HOME}/.local/llama.cpp-deepseek-v4-fringe-cuda-sm120}"
    export LLAMACPP_REPO="${LLAMACPP_REPO:-https://github.com/Fringe210/llama.cpp-deepseek-v4-flash-cuda.git}"
    export LLAMACPP_REF="${LLAMACPP_REF:-main}"
...
# Keep frontier runs on explicit VRAM/CPU-offload boundaries by default.
# Unified memory makes 96 GB cards appear to have impossible "free device
# memory" and undermines startup/OOM probes.
export GGML_CUDA_ENABLE_UNIFIED_MEMORY="${GGML_CUDA_ENABLE_UNIFIED_MEMORY:-0}"
export LLAMACPP_CONTEXT="${LLAMACPP_CONTEXT:-131072}"
...
export LLAMACPP_NO_MMAP="${LLAMACPP_NO_MMAP:-false}"
export LLAMACPP_FIT="${LLAMACPP_FIT:-}"
export LLAMACPP_CACHE_RAM="${LLAMACPP_CACHE_RAM:-}"
