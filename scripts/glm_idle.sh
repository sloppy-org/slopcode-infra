...
#
# Idle means, sustained for IDLE_SECONDS on BOTH faepmac1 and faepmac2:
#   - no interactive input (HID idle >= IDLE_SECONDS): screen locked, logged
#     out, or a human sitting inactive all count as idle;
...
#
# A bare ssh/tmux session that generates no load is idle by design.
#
# check   exit 0 when idle-and-sustained, 1 otherwise; one-line reason on stderr
...
case "${ACTION}" in
  check)
    ...
    if instant_idle; then
      ...
      if (( held >= IDLE_SECONDS )); then
        echo "idle: ${REASON}, sustained ${held}s" >&2
        exit 0
      fi
  ...
  status)
    ...
    echo "threshold: IDLE_SECONDS=${IDLE_SECONDS} LOAD_FRAC=${LOAD_FRAC}"
