...
h = hashlib.sha256()
h.update(b"slopcode-opencode-prewarm-v2\0")
...
if [[ "${CHECK_ONLY}" == "true" ]]; then
  if [[ "${OLD_FINGERPRINT}" == "${CURRENT_FINGERPRINT}" ]]; then
    echo "fresh: ${MANIFEST}"
    exit 0
  fi
  echo "stale: ${MANIFEST}"
...
wait_for_server() {
  local timeout="${SLOPCODE_PREWARM_WAIT_TIMEOUT:-900}"
  local deadline=$(( $(date +%s) + timeout ))
  while ! server_ready; do
    [[ $(date +%s) -ge ${deadline} ]] && die "llama-server did not become ready at ${BASE_URL}"
    sleep 2
  done
}
...
python3 - "${MANIFEST}" "${CURRENT_FINGERPRINT}" "${OPENCODE_VERSION}" "${LLAMA_VERSION}" "${WATCH_PATHS}" "${BASE_URL}" <<'PY'
import json
import sys
from datetime import datetime, timezone

path, fingerprint, opencode_version, llama_version, watch_paths, base_url = sys.argv[1:]
data = {
...
echo "prewarm complete: ${MANIFEST}"
