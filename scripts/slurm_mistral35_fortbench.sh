#!/usr/bin/env bash
# Run Mistral Medium 3.5 UD-Q4_K_XL for FortBench inside one Slurm job.
...
RUN_DIR="${RUN_DIR:-${RUN_ROOT}/mistral35-udq4-128k-${MODE}-${JOB_ID}-${STAMP}}"
mkdir -p "${RUN_DIR}"
...
cleanup() {
  ...
  if [[ -n "${FORTBENCH_LITELLM_PROXY_PORT:-}" ]]; then
...
export LLAMACPP_HOME="${LLAMACPP_HOME:-${HOME}/.local/llama.cpp-cuda-sm120}"
...
export LLAMACPP_MODEL_ALIAS="${LLAMACPP_MODEL_ALIAS:-mistral-medium-3.5-ud-q4}"
...
export LLAMACPP_CMAKE_EXTRA="${LLAMACPP_CMAKE_EXTRA:--DCMAKE_CUDA_ARCHITECTURES=120 -DLLAMA_CURL=OFF}"

export FORTBENCH_LOCAL_USER="${FORTBENCH_LOCAL_USER:-${USER}}"
export FORTBENCH_LITELLM_PROXY_PORT="${FORTBENCH_LITELLM_PROXY_PORT:-4000}"
export FORTBENCH_LITELLM_REQUEST_TIMEOUT_SECONDS="${FORTBENCH_LITELLM_REQUEST_TIMEOUT_SECONDS:-3600}"
export FORTBENCH_RUNTIME_DIR="${FORTBENCH_RUNTIME_DIR:-${HOME}/.local/fortbench-runtime-py311}"
...
if [[ "${MODE}" == "smoke" ]]; then
  SUITE="${FORTBENCH_DIR}/suites/mistral-medium-3.5-smoke.yaml"
else
  SUITE="${FORTBENCH_DIR}/suites/mistral-medium-3.5-corpus20.yaml"
fi
