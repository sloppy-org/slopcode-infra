...
case "$1 $2" in
  ...
  "pr list")
    r="$(argval -R "$@")"
    if   [[ "$all" == *headRefName,body* ]]; then cat "$FIX/prref_$(slug "$r").txt" 2>/dev/null
...
reset_fixtures() { rm -f "${FIX}"/*.txt "${FIX}"/*.json; rm -f "${RUN_DIR}"/glm-repos-*.cache; : >"${POSTED_LOG}"; }

# review-needed PR view: a commit, no prior ORCHESTRATE review comment.
prview_needs() { printf '{"commits":[{"committedDate":"2026-06-01T00:00:00Z"}],"comments":[]}' >"${FIX}/prview_${1}_${2}.json"; }
# reviewed-after-commit PR view: commit older than the last review comment.
prview_done() { printf '{"commits":[{"committedDate":"2026-06-01T00:00:00Z"}],"comments":[{"createdAt":"2026-06-02T00:00:00Z","body":"ORCHESTRATE_REVIEW_STATUS: approve"}]}' >"${FIX}/prview_${1}_${2}.json"; }
...
test_group_fallthrough() {
  ...
  printf 'foo/x\n' >"${FIX}/repos_foo.txt"
  : >"${FIX}/propen_foo_x.txt"; printf '5\n' >"${FIX}/issues_foo_x.txt"; : >"${FIX}/prref_foo_x.txt"
  local out; out="$(next_action)"
...
test_review_beats_new_work_across_groups() {
  ...
  reset_fixtures; TARGET_GROUPS=(itpplasma foo)
  printf 'itpplasma/a\n' >"${FIX}/repos_itpplasma.txt"
  : >"${FIX}/propen_itpplasma_a.txt"; printf '5\n' >"${FIX}/issues_itpplasma_a.txt"; : >"${FIX}/prref_itpplasma_a.txt"
  printf 'foo/x\n' >"${FIX}/repos_foo.txt"
...
test_skip_issue_with_existing_pr() {
  echo "TEST: an issue that already has an open PR is not picked for implement"
  reset_fixtures
  printf '5\n7\n' >"${FIX}/issues_itpplasma_a.txt"
  printf 'fix/issue-5\nsome body\n' >"${FIX}/prref_itpplasma_a.txt"
...
test_skip_issue_referenced_by_closes() {
  echo "TEST: 'Closes #N' in a PR body also excludes issue N"
  reset_fixtures
  printf '8\n9\n' >"${FIX}/issues_itpplasma_a.txt"
...
test_needs_review_stale_after_commit() {
  echo "TEST: a PR already reviewed after its last commit does not need review"
  reset_fixtures; prview_done itpplasma_a 1
...
test_do_review_posts_status() {
  ...
  reset_fixtures
  printf 'reasoning line\n{"review_status":"request_changes","summary":"x","findings":[]}\ntrailer\n' >"${ROOT}/mout"
...
test_do_review_approve() {
  ...
  reset_fixtures
  printf '{"review_status":"approve","summary":"ok","findings":[]}\n' >"${ROOT}/mout"
...
test_installer_dry() {
  echo "TEST: launchd installer dry-run writes a RunAtLoad + StartInterval plist"
  ...
  if grep -q 'com.slopcode.glm-autonomous' "$p" 2>/dev/null \
     ...
     && grep -q 'StartInterval' "$p" 2>/dev/null \
     && grep -q '<string>run</string>' "$p" 2>/dev/null; then
    rm -rf "$tmp"; pass
  else rm -rf "$tmp"; fail "plist missing keys"; fi
}
...
test_needs_review_stale_after_commit || FAILED=$((FAILED + 1))
